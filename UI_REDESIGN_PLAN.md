...
- **Admin chrome redesigned (this iteration):** `admin/layouts/app` moved from dark-sidebar/white-content to a light premium shell — warm-paper (`surface-50`) fixed sidebar with hairline border, ink rounded-square logo mark with gold "P", `admin-nav-link` pattern (ink pill active state, warm hover), shared `<x-icon>` set (33 inline SVGs deleted), `<x-flash>`, styleguide shortcut in the topbar; admin dashboard rebuilt with KPI cards (gold revenue tile), linked content-library strip, `x-status-badge` (inline `@if` badge chains deleted), gold unread markers. Verified: all 22 referenced routes exist, view-cache compiles, admin pages live-200 under the new chrome, mobile hamburger + overlay behavior preserved.
- **Customer pages complete (this iteration):** all 7 account pages (orders index/show, quotes index/show, purchases, payments, notifications) rebuilt on the warm-paper system — `page-hero-light` headers with breadcrumbs, `surface-card` panels, `x-status-badge` everywhere (inline status maps deleted; added `failed`/`refunded` to the badge map), `x-empty-state` for zero-data, form controls → `form-control-modern`, buttons → `.btn` system, order timeline emoji → icon dots, chat bubbles indigo → ink/gold. Verified: all 19 referenced routes exist, view-cache compiles, logged-in browser check on live routes (200 + rendered). Every customer-facing page now matches the dashboard.
- **Asset & brand sweep (this iteration):** full favicon/tile set (16/32/180/192/512/144 + multi-size .ico) was still the old indigo brand and `favicon.ico` was a 0-byte file — all regenerated in the current ink + gold mark via `tools/generate-icons.php` (rerunnable); `site.webmanifest` `theme_color` indigo → gold; email templates (`emails/layout`, `emails/notification`) re-branded from indigo gradient buttons to dark-ink header + gold CTA. Verified: every DB-referenced image exists on disk, all icons pass `getimagesize`, CSS/JS/storage all serve 200 via the `photolabe.local` vhost, `optimize:clear` run after view-cache pass.
- **Consistency sweep (this iteration):** all 6 error pages re-skinned to the warm-paper system via a new shared `errors/layout` (old indigo design fully retired); new shared `<x-flash>` component replaces every hand-rolled session alert (frontend layout, admin layout, cart, checkout, contact, auth, customer order/quote show); auth screens de-emojied, brand panels moved from muddy espresso/gold gradients to surface-950 + gold mesh, low-contrast `text-primary-500` links fixed; customer dashboard + profile rebuilt on the design system (surface-card, x-status-badge, form-control-modern, btn system, page-hero-light); stale `theme-color`/TileColor `#4c6ef5` → gold; header search overlay dead slug fixed (`photo-retouching` → `professional-photo-retouching`); header cart badge hardened for edge cases.
