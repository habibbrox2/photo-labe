...
<div class="flex items-center justify-between mb-6">
    ...
    @if($unreadCount > 0)
        <form method="POST" action="{{ route('admin.notifications.read-all') }}">
            @csrf
            <button type="submit" class="px-4 py-2 bg-primary-600 text-white text-sm font-semibold rounded-lg hover:bg-primary-700">
...
<div class="bg-white rounded-xl border border-gray-200 divide-y divide-gray-100 overflow-hidden">
    @forelse($notifications as $notification)
        <a href="{{ route('admin.notifications.open', $notification) }}" class="px-5 py-4 flex items-start justify-between hover:bg-gray-50 transition-colors">
            ...
            <span class="text-xs font-medium {{ $notification->read_at ? 'text-gray-400' : 'text-primary-600' }} flex-shrink-0 ml-4">
