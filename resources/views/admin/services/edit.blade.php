    ...
    <form method="POST" action="{{ route('admin.services.update', $service) }}" enctype="multipart/form-data">
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-6 space-y-5">
            ...
            <div>
                ...
                <textarea name="short_description" rows="2" class="w-full px-4 py-2.5 rounded-lg border border-gray-200 text-sm focus:border-indigo-500 outline-none">{{ old('short_description', $service->short_description) }}</textarea>
            </div>

            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-1">Description</label>
                <textarea name="description" rows="6" class="w-full px-4 py-2.5 rounded-lg border border-gray-200 text-sm focus:border-indigo-500 outline-none">{{ old('description', $service->description) }}</textarea>
            </div>
            ...
            <div>
                ...
                @if($service->featured_image)
                    <div class="mb-2"><img src="{{ asset('storage/' . $service->featured_image) }}" class="h-20 rounded-lg object-cover"></div>
                @endif
            ...
            <div class="flex items-center gap-2">
                <input type="checkbox" name="is_featured" value="1" {{ old('is_featured', $service->is_featured) ? 'checked' : '' }} class="rounded border-gray-300 text-indigo-600">
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-6 mt-4 space-y-5">
            ...
            <div>
                ...
                <textarea name="seo_description" rows="2" maxlength="500" class="w-full px-4 py-2.5 rounded-lg border border-gray-200 text-sm focus:border-indigo-500 outline-none">{{ old('seo_description', $service->seo_description) }}</textarea>
            </div>
        </div>

        <div class="flex items-center gap-3 mt-6">
            <button type="submit" class="px-6 py-2.5 bg-indigo-600 text-white text-sm font-semibold rounded-lg hover:bg-indigo-700 transition-colors">Update Service</button>
