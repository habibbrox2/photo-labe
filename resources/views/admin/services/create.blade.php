...
@section('content')
<div class="max-w-3xl">
    <x-breadcrumbs :items="[['label' => 'Services', 'url' => route('admin.services.index')], ['label' => 'Create Service']]" />
    <form method="POST" action="{{ route('admin.services.store') }}" enctype="multipart/form-data">
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-6 space-y-5">
            <div class="bg-primary-50/60 rounded-lg px-4 py-3 text-sm text-gray-700 border border-primary-100">
                ...
                — Add a service offered by the studio. Upload one <strong>featured image</strong> — shown on the services grid.
            </div>
            <div>
            ...
            <div>
                ...
                <textarea name="description" rows="6" class="w-full px-4 py-2.5 rounded-lg border border-gray-200 text-sm focus:border-accent-500 outline-none">{{ old('description') }}</textarea>
            </div>
        ...
        <div class="flex items-center gap-3 mt-6">
            <button type="submit" class="px-6 py-2.5 bg-primary-600 text-white text-sm font-semibold rounded-lg hover:bg-primary-700 transition-colors">Create Service</button>
